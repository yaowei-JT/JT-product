...
**功能描述**：以卡片形式展示8个核心指标，直观呈现当前筛选条件下的采购交付概况，分为两行展示，每行4个卡片。
...
| styleCount | number | ≥0 | 150 |
| orderTotalQuantity | number | ≥0 | 30000 |
| remainingQuantity | number | ≥0 | 2000 |
| avgDeliveryDays | string | 数字或"-" | "12.5" |

**异常场景处理**：

- 数据加载中：显示loading动画
- 无数据：所有卡片显示"0"，准交率和平均交期天数显示"-"
...
| 下单款数 | 采购订单的下单款号去重计数 |
| 下单总件数 | 采购订单的下单总件数求和 |
| 剩余待入库件数 | 应入库件数 - 实际入库件数，显示为红色 |
| 平均交期天数 | 采购订单的实际交货天数求平均，应交为0时显示"-" |
...
| 汇总行 | 表格底部显示合计行，包含所有指标的总计值，背景为灰色，字体加粗 |
| 汇总数据 | 合计行数据与统计概览区数据一致 |
